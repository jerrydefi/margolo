...
{
    MargoloNFT public margoloNFT;

    function initialize(address margoloNFT_) public virtual initializer {
        require(margoloNFT_ != address(0), 'ICP0');
        __Ownable_init_unchained();
        margoloNFT = MargoloNFT(margoloNFT_);
    }
    ...
    function createAccount() public virtual returns (address) {
        ...
        bytecode = abi.encodePacked(bytecode, abi.encode(address(this), address(margoloNFT)));
    ...
    // 从函数签名到实现它们的连接器的折叠连接器注册和 IFoldingConnectorProvider 映射
    ...
    function changeCTokenFromPlatform(
        address platform,
        address token,
        address newSynthToken
    ) external virtual onlyOwner {
        require(platform != address(0), 'FR14');
        require(token != address(0), 'FR14');
        require(tokensOnPlatforms[platform][token] != address(0), 'FR15');
        tokensOnPlatforms[platform][token] = newSynthToken;
        emit TokenOnPlatformUpdated(platform, token, newSynthToken);
    }
