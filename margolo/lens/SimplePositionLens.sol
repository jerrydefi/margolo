...
contract SimplePositionLens {
    ...
    function getAllPositionsFromNFT(address foldingNFT)
        external
        returns (SimplePositionMetadata[] memory assetsData)
    {
        uint256 totalPositions = ERC721(foldingNFT).totalSupply();
        assetsData = new SimplePositionMetadata[](totalPositions);

        for (uint256 i = 0; i < totalPositions; i++) {
            assetsData[i] = ISimplePositionBaseConnector(ERC721(foldingNFT).tokenByIndex(i))
                .getPositionMetadata();
        }
    }
}